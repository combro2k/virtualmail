...
RUN touch ${INSTALL_LOG} && \
    ...
    apt-get update 2>&1 | tee -a ${INSTALL_LOG} > /dev/null && apt-get dist-upgrade -y 2>&1 | tee -a ${INSTALL_LOG} > /dev/null && apt-get install -y libberkeleydb-perl libnet-dns-perl libnet-server-perl \
...
RUN /usr/bin/freshclam --config-file=/etc/clamav/freshclam.conf 2>&1 | tee -a ${INSTALL_LOG} > /dev/null

# Spamassassin
ADD resources/spamassassin /etc/spamassassin
RUN cpan -f install Mail::SPF::Query 2>&1 | tee -a ${INSTALL_LOG} > /dev/null && cpan -f install Mail::SpamAssassin 2>&1 | tee -a ${INSTALL_LOG} > /dev/null && \
...
RUN ln -s /usr/bin/nodejs /usr/bin/node && \
    ...
    /opt/mailman/bin/python -c 'import pip, subprocess; [subprocess.call("/opt/mailman/bin/pip install --pre -U " + d.project_name, shell=1) for d in pip.get_installed_distributions()]' 2>&1 | tee -a ${INSTALL_LOG} > /dev/null && \
    virtualenv --system-site-packages -p python2.7 /opt/postorius 2>&1 | tee -a ${INSTALL_LOG} > /dev/null && \
    /opt/postorius/bin/pip install -U --pre django-gravatar flup postorius Whoosh mock beautifulsoup4 hyperkitty python-openid python-social-auth django-browserid 2>&1 | tee -a ${INSTALL_LOG} > /dev/null && \
